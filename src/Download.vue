    ...
    methods: {
        ...
        detectOS() {
            ...
            this.downloadLinks = {
                ...
                Linux: {
                    Flatpak:
                        "https://flathub.org/en/apps/com.beavernotes.beavernotes",
...
<template>
    <div
        class="min-h-screen mx-auto max-w-2xl pt-32 space-y-6 px-4 text-center"
    >
        <!-- Logo -->
        ...
        <div v-if="!isMobile" class="mt-8 flex justify-center">
            <div class="relative w-full max-w-md">
                ...
                <div class="absolute top-0 left-0 flex items-center gap-1">
                    ...
                    <ul
                        v-if="showOSDropdown"
                        class="absolute mt-6 w-32 bg-white dark:bg-neutral-800 border rounded-md shadow-lg z-50 overflow-hidden"
                    >
                        <li
                            v-for="os in osOptions"
                            :key="os.name"
                            @click="selectOS(os.name)"
                            class="px-2 py-1 hover:bg-primary/20 cursor-pointer flex items-center gap-1 text-sm"
                        >
                            <component
                                :is="os.icon || 'RiTerminalLine'"
                                class="w-4 h-4"
                            />
                            {{ os.name }}
                        </li>
                ...
                >
                    <!-- Scrollable command -->
                    <div
                        class="flex-1 overflow-x-auto whitespace-nowrap no-scrollbar"
                    >
                        {{ cliCommand }}
                    </div>
